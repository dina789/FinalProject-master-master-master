...
//google static map
        ...
        Picasso.with(context).load(src).into(imagemap );
        //Picasso.with(getContext()).load("https://maps.googleapis.com/maps/api/staticmap?center=40.714728,-73.998672&zoom=12&size=400x400&key=YOUR_API_KEY";
        ImageView imageViewmapapi = (ImageView) findViewById(R.id.imageViewmapapi);

        Picasso.with(context).load(src).into(imageViewmapapi );
        //Picasso.with(getContext()).load("maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=CnRtAAAATLZNl354RwP_9UKbQ_5Psy40texXePv4oAlgP4qNEkdIrkyse7rPXYGd9D_Uj1rVsQdWT4oRz4QrYAJNpFX7rzqqMlZw2h2E2y5IKMUZ7ouD_SlcHxYq1yL4KbKUv3qtWgTK0A6QbGh87GB3sscrHRIQiG2RrmU_jF4tENr9wGS_YxoUSSDrYjWmrNfeEHSGSc3FyhNLlBU&key=AIzaSyByy9wb2_lp8zUZSlDBVFgisRmyAVIsjXM" + placemodel.getFormat(format)).into(image);
